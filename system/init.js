...
exports.initDAL = (config) => {
	DAL_connections = DAL.init(config);
...
(() => {
	...
	for(let file of modules){
		try{
			...
			if(module.__init){
				exports.inits[file] = module.__init;
			}
			...
			for(let m in module){
				...
				if(m.indexOf('_') === 0){
					...
					if(exports.modules[methodName]){
						log.e('module', moduleName, 'Method', methodName, 'in file', file, 'IS DEFINED IN', exports.modules[methodName].definedIn);
						continue;
					}

					exports.modules[methodName] = method;

					let aliasURL = methodMeta.alias;
					if(aliasURL){
						aliasURL = getUrl(aliasURL, methodMeta, module, moduleName);
						if(exports.modules[aliasURL]){
							log.e('module', moduleName, 'Method', aliasURL, 'in file', file, 'IS DEFINED IN', exports.modules[aliasURL].definedIn);
							continue;
						}

						exports.modules[aliasURL] = method;
					}
				}
			}
			module = null;
	...
	//log.w(Object.keys(exports.modules))
	...
	for(let ii in exports.inits){
		try{
			exports.inits[ii](context, function(){});
		}catch(e){
			log.e('__init()', ii, e);
		}
 	}
