...
* `useDals`: An object of dal modules which need to include. Last version supports redis(for use specify `redis:{}`), mysql(for user specify `mysql:{}` and default will be `{host: '127.0.0.1',user: 'root'}`), postgress(for user specify `postgress:{}` and default will be `{host: '127.0.0.1',user: 'root'}`). For built-in redis avaliable params [see here](https://github.com/NodeRedis/node_redis#options-object-properties), mysql [see here](https://github.com/mysqljs/mysql#connection-options), postgres [see here](https://github.com/brianc/node-postgres/wiki/Client#parameters) (Example of dals config: `useDals: {mysql: {port: 6375, user: 'admin'}, redis: {}}`)
